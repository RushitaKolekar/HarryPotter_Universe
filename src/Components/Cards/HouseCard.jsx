    ...
    <div className="min-h-[60vh] py-2 flex flex-col items-center relative">
      ...
      <div className="w-4/5 mx-auto group"> {/* Same width as slider with left padding */}
        <motion.h2
          ...
          className="text-3xl md:text-4xl font-bold text-transparent bg-clip-text pb-5 bg-gradient-to-r from-purple-400 to-emerald-500 
                    transform transition-all duration-700 group-hover:scale-105 group-hover:translate-y-1"
        >
          Hogwarts House Legacy
        </motion.h2>
      </div>
      ...
      <div className="w-full relative ">
        ...
        >
          <div className="h-16 w-16 bg-white dark:bg-gray-800 rounded-full shadow-lg flex items-center justify-center mr-4">
            <svg className="w-8 h-8 text-emerald-600 dark:text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {HogwartHouseInfo.map((house) => (
            ...
            >
              <div className="group relative bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 h-full border-2 border-transparent hover:border-purple-500 dark:hover:border-purple-400">
                ...
                {/* Movie Info */}
                <div className="p-3">
                  <NavLink to={`/movies/${house.id}`}>
                    <motion.p 
                      className="text-xl font-bold text-gray-800 dark:text-white pb-2 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors"
                      whileHover={{ x: 5 }}
                    >
                      {house.name}
                    </motion.p>
                    ...
                    <motion.p className='italic text-yellow-500 font-bold pb-5'>
                      {house.house_quote}
                    </motion.p>

                  <NavLink to={`/houses/${house.id}`}>
                    <motion.button
